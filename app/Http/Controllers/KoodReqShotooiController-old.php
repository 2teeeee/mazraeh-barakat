...
class KoodReqShotooiController extends Controller
{   
...
//            "btn"=>$btn,
    ...
    {       
     //   return Redirect::to('lock.html');
        ...
        $value = ShotooiValue::where('product_id',$id)->where('user_id',Auth::id())->sum('value');
		...
		if($id == 1)
		{
			$kise = round(($value / 13) /50);
		}
        

        // load the create form (app/views/samRequest/create.blade.php)
        return view("admin.request.koodReqShotooi.create")->with([
            'sendType' => $sendType,
            'koods' => $koods,
            'brokers' => $brokers,
            'value' => $value,
    ...
    public function store(Request $request)
    {
        ...
        // process the login
        if ($validator->fails()) {
            return Redirect::to('koodReqShotooi/create/'.$prod)
      ...
      //      $numbReq = Input::get('numb');
            ...
            foreach(Cart::getContent() as $item):
                ...
                if(($productCheck == $prod) and ($koodCheck == $koodId) and ($ct_id == Auth::user()->city->ct_id))
                {
                    ...
                    return Redirect::to('koodReqShotooi/cart');
                }
            
            endforeach;
			$value = ShotooiValue::where('product_id',$prod)->where('user_id',Auth::id())->sum('value');
            ...
            if($numbReq > 0)
            {
                if($checkBroker)
                {
                    ...
                    $model->ct_id = Auth::user()->city->ct_id;
    ...
    {
        return View('admin.request.koodReqShotooi.cart');
    }
...
//        }
    ...
    //        $koodReq = KoodReq::find($item->id);
    //        $koodReq->status = 1;
    //        $koodReq->make_date = now();
    //        $koodReq->save();
