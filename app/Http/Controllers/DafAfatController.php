...
class DafAfatController extends Controller
{
    ...
    {
        $request->user()->authorizeRoles(['admin','manager','programmer','operator','pestAndDisinfectionCompany']);
        
        return view("admin.dafAfat.index")->with([]);
    }
    ...
    public function all(Request $request)
    {
        ...
        $crumb[1] = ['title'=>'مدیریت','url'=>'\dafAfat\all','class'=>'active'];

        $btn[0] = ['title'=>'جدید','url'=>'\dafAfat\create','class'=>'btn-success','icon'=>'pencil'];
        
        $Grid = new Grid(DafAfat::query(), 'dafAfats');
        ...
        // load the view and pass the dafAfat
    	return view("admin.dafAfat.all")->with([
            "model"=>$model,
            "crumb"=>$crumb,
            "btn"=>$btn,
            "grid"=>$Grid
    ...
    public function create(Request $request)
    {
        ...
        $crumb[0] = ['title'=>'صفحه اصلی','url'=>'admin','class'=>''];
        $crumb[1] = ['title'=>'مدیریت ;کاربر ها','url'=>'\dafAfat\all','class'=>''];
        $crumb[2] = ['title'=>'کاربر جدید','url'=>'\dafAfat\create','class'=>'active'];

        $btn[0] = ['title'=>'مدیریت','url'=>'/dafAfat/all','class'=>'btn-info','icon'=>'book'];

        $users = User::all();

        // load the create form (app/views/dafAfat/create.blade.php)
        return view("admin.dafAfat.create")->with([
            "crumb"=>$crumb,
            "btn"=>$btn,
            'users'=>$users
        ]);
    ...
    public function store(Request $request)
    {
        ...
        // process the login
        if ($validator->fails()) {
            return Redirect::to('dafAfat/create')
            ...
            if($image != null)
            {
                ...
                $location = public_path('image/dafAfat/' . $filename);
            ...
            return Redirect::to('dafAfat/all');
        }
    }
    ...
    public function show($id,Request $request)
    {
        ...
        // show the view and pass the dafAfat to it
        return View('admin.dafAfat.show')->with(
            'model', $model
        );
    }
    ...
    public function edit($id,Request $request)
    {
        ...
        $btn[0] = ['title'=>'مدیریت','url'=>'/dafAfat/all','class'=>'btn-info','icon'=>'book'];
        $btn[1] = ['title'=>'جدید','url'=>'/dafAfat/create','class'=>'btn-success','icon'=>'pencil'];
        
        // show the edit form and pass the dafAfat
        return View('admin.dafAfat.edit')->with([
            'model'=>$model,
            'users'=>$users,
            'btn'=>$btn
        ]);
    ...
    public function update($id,Request $request)
    {
        ...
        // process the login
        if ($validator->fails()) {
            return Redirect::to('dafAfat/' . $id . '/edit')
                ->withErrors($validator)->withInput();
        } else {
            // store
            $model = DafAfat::find($id);
    ...
    public function newRequest(Request $request)
    {
        ...
        // load the create form (app/views/dafAfat/create.blade.php)
        return view("admin.dafAfat.new_request")->with([
            "crumb"=>$crumb,
            "btn"=>$btn,
            'types'=>$types,
            'tab' => 0
    ...
    public function saveRequest(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            // redirect
            Session::flash('message', trans('validation.attributes.success_save'));
            return Redirect::to('dafAfat/list');
        }
    }
    ...
    public function listBah(Request $request)
    {
    	...
    	// get all the dafAfat
    	$model = Location::where('type_id',156)->where('user_id',Auth::id())->get();
        ...
        $gridQuery = Location::join('users','users.id', '=', 'locations.user_id')
                ...
                ->where('type_id',156)->where('user_id',Auth::id());
     ...
     //   $Grid->action(trans('validation.attributes.edit'), '{id}/edit');
        ...
        // load the view and pass the dafAfat
    	return view("admin.dafAfat.list")->with([
            "model"=>$model,
            "crumb"=>$crumb,
            "btn"=>$btn,
            "grid"=>$Grid,
