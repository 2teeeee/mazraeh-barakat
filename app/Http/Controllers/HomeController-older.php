...
public function testLang()
{
    ...
    public function test()
    {
		...
		foreach($ml as $co)
		{
			$us = User::where('codemelli',$co->codemeli)->first();
			if($us != null)
			{
				$i++;
				$us->blockKood = 1;
	...
	//	return ' => ok...';
		...
		Sms::sendSMS("پیام ارسال شد","09173326706");
                 echo 'send'; 
...
//		{
	//		if($m->kesht)
	//		{
	//			$req = KoodReq::find($m->id);
	//			$req->sath = $m->kesht->square;
	//			$req->save();
	//		}
	//	}
		
	//	return; 
      //  $ch = Check::whereNull('che')->get();
...
// 6839792196
        ...
        if($response != null)
            return view('test')->with(['model' => $response]);
        else 
            return;  */
//        foreach ($ch as $value) {
//           
...
//                $b = 0;
//                foreach($response['GetPahneFarmInfoResult']['Products']['Product'] as $prod)
//                {
//                    $b = $b + (int)$prod['Square'];
//                }
//            }
