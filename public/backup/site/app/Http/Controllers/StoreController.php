...
class StoreController extends Controller
{
    ...
    public function all(Request $request)
    {
        ...
        $crumb[1] = ['title'=>'مدیریت','url'=>'\store\all','class'=>'active'];

        $btn[0] = ['title'=>'جدید','url'=>'\store\create','class'=>'btn-success','icon'=>'pencil'];
    	...
    	return view("admin.store.all")->with([
            "model"=>$model,
            "crumb"=>$crumb,
            "btn"=>$btn,
            "grid"=>$Grid
    ...
    public function create(Request $request)
    {
        ...
        $crumb[0] = ['title'=>'صفحه اصلی','url'=>'admin','class'=>''];
        $crumb[1] = ['title'=>'مدیریت ;کاربر ها','url'=>'\store\all','class'=>''];
        $crumb[2] = ['title'=>'کاربر جدید','url'=>'\store\create','class'=>'active'];

        $btn[0] = ['title'=>'مدیریت','url'=>'/store/all','class'=>'btn-info','icon'=>'book'];

        $users = User::all();

        // load the create form (app/views/store/create.blade.php)
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if($image2 != null)
            {
                ...
                $location2 = public_path('image/store/' . $filename);
    ...
    public function show($id,Request $request)
    {
        ...
        return View('admin.store.show')->with(
            'model', $model
        );
    }
    ...
    public function edit($id,Request $request)
    {
        ...
        $btn[0] = ['title'=>'مدیریت','url'=>'/store/all','class'=>'btn-info','icon'=>'book'];
        $btn[1] = ['title'=>'جدید','url'=>'/store/create','class'=>'btn-success','icon'=>'pencil'];
    ...
    public function newRequest(Request $request)
    {
        ...
        // load the create form (app/views/store/create.blade.php)
        return view("admin.store.new_request")->with([
            "crumb"=>$crumb,
            "btn"=>$btn,
            'types'=>$types,
            'tab' => 0
    ...
    public function listBah(Request $request)
    {
    	...
    	// get all the store
    	$model = Location::where('type_id',153)->where('user_id',Auth::id())->get();
        ...
        $gridQuery = Location::join('users','users.id', '=', 'locations.user_id')
                ...
                ->where('type_id',153)->where('user_id',Auth::id());
     ...
     //   $Grid->action(trans('validation.attributes.edit'), '{id}/edit');
        ...
        // load the view and pass the store
    	return view("admin.store.list")->with([
            "model"=>$model,
            "crumb"=>$crumb,
            "btn"=>$btn,
            "grid"=>$Grid,
