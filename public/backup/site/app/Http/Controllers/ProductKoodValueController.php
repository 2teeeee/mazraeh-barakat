...
class ProductKoodValueController extends Controller
{
    ...
    {
        $request->user()->authorizeRoles(['admin','manager','programmer','takhsis','nazer']);
        ...
        if(Auth::user()->hasRole('managerJahad'))
        {
            $query = ProductKoodValue::join('products','products.code','=','product_kood_values.productCode_id')
                ...
                ->where('product_kood_values.ct_id',Auth::user()->city->ct_id)
                ->orderBy('id','desc');
        }
        else
        {
...
//            ->
    ...
    {
        $request->user()->authorizeRoles(['admin','manager','programmer','takhsis']);
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
			...
			$model->userKeshtReqStatus = Input::get("userKeshtReqStatus")?1:0;
