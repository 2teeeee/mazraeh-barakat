...
<div class="col-sm-12">
    ...
    <div class="panel panel-default">
        <div class="panel-body border-bottom">
			<form method="POST" action="{{ route('report/ostanReport') }}">
                        @csrf
                <div class="card-body px-0">
                    <div class="row">
                        <div class="form-group col-md-2 p-0">
                            <label for="ostan_id" class="col-md-12 col-form-label text-md-right">{{ __('validation.attributes.ostan_id') }} </label>

                            <div class="col-md-12">
                                <select name="ostan_id" id="ostan_id" class="form-control {{ $errors->has('ostan_id') ? ' is-invalid' : '' }}">
                                    <option disabled selected value>استان مورد نظر را انتخاب کنید</option>
                                    @foreach ($ostans as $ostan)
                                        <option {{ (old('ostan_id') == $ostan->id)?'selected':(($ostan_id == $ostan->id)?'selected':'') }} value="{{ $ostan->id }}">{{ $ostan->title }}</option>
                                    @endforeach
                                </select>
						...
						<div class="form-group col-md-2 p-0">
                            ...
                            <div class="col-md-12">
                                <select name="city_id" id="city_id" class="form-control {{ $errors->has('city_id') ? ' is-invalid' : '' }}">
                                    <option selected value>همه ی شهرستان ها</option>
                                    @foreach ($citys as $city)
                                        <option {{ (old('city_id') == $city->id)?'selected':(($city_id == $city->id)?'selected':'') }} value="{{ $city->id }}">{{ $city->title }}</option>
                                    @endforeach
                                </select>
        ...
        <div class="panel-body mt-2 table-responsive" id="response">
			@component('admin.report._responseOstanReport', [
				'citys' => $cityViews, 
				'kood_id' => $kood_id,
